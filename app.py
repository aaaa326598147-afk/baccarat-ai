...
def get_final_analysis(history, win_streak):
    if len(history) < 5: return "⏳ 數據校準中...", random.randint(31, 55)
    path = "".join(history[-10:]); last_4 = "".join(history[-4:])
    
    # 根據規律設定信心度基礎值
    if "莊莊莊莊" in path or "閒閒閒閒" in path: base_conf = random.randint(92, 98)
    elif last_4 in ["莊莊閒閒", "閒閒莊莊", "莊閒莊閒", "閒莊閒莊"]: base_conf = random.randint(81, 91)
    else: base_conf = random.randint(34, 58) # 雜亂盤勢必定低於 60
...
# --- 2. 嚴格 CSS (徹底鎖定顏色與移除陰影) ---
st.set_page_config(page_title="VIP AI-Pro V12.6", layout="centered")
