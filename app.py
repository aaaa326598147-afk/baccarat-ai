...
# --- 2. 專業 CSS ---
st.set_page_config(page_title="VIP AI-Pro V13.0", layout="centered")
...
# 設定人數：初始 182-235，每次操作後隨機增減 1-5 人
if 'viewers' not in st.session_state: 
    st.session_state.viewers = random.randint(182, 235)
else:
    st.session_state.viewers += random.choice([-3, -2, -1, 1, 2, 3, 5])
    # 確保人數維持在合理區間
    st.session_state.viewers = max(min(st.session_state.viewers, 250), 160)
